namespace _13_PathFinding
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            List<Datastruct.Point> path;

            Datastruct.AStar.PathFinding(tileMap, new Datastruct.Point(1, 1), new Datastruct.Point(1, 7), out path);
            PrintResult(tileMap, path);
        }
        static void PrintResult(in bool[,] tileMap, in List<Datastruct.Point> path)
        {
            ...
            foreach (Datastruct.Point point in path)
            {
                pathMap[point.y, point.x] = '*';
            }

            Datastruct.Point start = path.First();
            Datastruct.Point end = path.Last();
